...
class DiscordYoinkClient(commands.Bot):
    ...
    async def download_attachment(self, attachment: discord.Attachment, save_path: str) -> bool:
    ...
    async def get_channel_history(self, channel: discord.TextChannel, limit: Optional[int] = None) -> List[discord.Message]:
    ...
    async def get_channel_info(self, channel: discord.abc.GuildChannel) -> Dict[str, Any]:
        ...
        if isinstance(channel, discord.TextChannel):
            ...
            channel_info.update({
                'channels': [str(ch.id) for ch in channel.channels]
            })
    ...
    async def get_member_info(self, member: discord.Member) -> Dict[str, Any]:
        ...
        return {
            ...
            'roles': [str(role.id) for role in member.roles if role.id != member.guild.default_role.id],
    ...
    async def get_message_info(self, message: discord.Message) -> Dict[str, Any]:
        ...
        message_info = {
            ...
            'author': await self.get_member_info(message.author) if isinstance(message.author, discord.Member) else {
            ...
            for reaction in message.reactions:
                ...
                try:
                    async for user in reaction.users():
                        ...
                        if len(reaction_info['users']) >= 100:  # Limit to prevent excessive API calls
                            break
                except:
                    pass
        ...
        if message.reference:
            message_info['reference'] = {
                ...
                'type': 'reply' if message.type == discord.MessageType.reply else 'reference'
            }
            
            # Check if this is a cross-server forward
            is_cross_server = (message.reference.guild_id and 
                             str(message.reference.guild_id) != str(message.guild.id) if message.guild else False)
            
            if is_cross_server:
                message_info['reference']['cross_server'] = True
                # Add explanation for empty content
                if not message_info['content']:
                    message_info['reference']['note'] = "Original text content unavailable for cross-server forwarded messages"
            
            # Try to get the referenced message content (for same-server forwards)
            try:
                if (message.reference.resolved and 
                    hasattr(message.reference.resolved, 'content') and
                    isinstance(message.reference.resolved, discord.Message)):
                    referenced_msg = message.reference.resolved
                    message_info['reference']['original_content'] = referenced_msg.content
