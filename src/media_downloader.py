...
class MediaDownloader:
            ...
            async with session.get(url) as response:
                ...
                if content_length:
                    ...
                    if size_mb > max_size_mb:
                        logger.warning(f"Skipping large file {filename}: {size_mb:.2f}MB")
                        return None
    ...
    async def download_image(
        ...
        return await self.download_file(url, Path(relative_path).name, base_dir / Path(relative_path).parent)
    ...
    async def download_voice_message(
        ...
        return await self.download_file(url, filename, base_dir / "voice", max_size_mb=50)
